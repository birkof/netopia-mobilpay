...
abstract class Mobilpay_Payment_Request_Abstract
{
    ...
    static public function factoryFromEncrypted($envKey, $encData, $privateKeyFilePath, $privateKeyPassword = null)
    {
        ...
        if ($privateKeyPassword == null) {
            $privateKey = @openssl_get_privatekey(
                is_file($privateKeyFilePath) && is_readable($privateKeyFilePath) ? "file://{$privateKeyFilePath}" : $privateKeyFilePath
            );
        } else {
            $privateKey = @openssl_get_privatekey(
                is_file($privateKeyFilePath) && is_readable($privateKeyFilePath) ? "file://{$privateKeyFilePath}" : $privateKeyFilePath,
    ...
    static protected function _factoryFromXml(DOMDocument $xmlDoc)
    {
        ...
        if ($elems->length != 1) {
            throw new Exception(
                'factoryFromXml order element not found', Mobilpay_Payment_Request_Abstract::ERROR_FACTORY_BY_XML_ORDER_ELEM_NOT_FOUND
            );
        }
        ...
        if ($attr == null || strlen($attr->nodeValue) == 0) {
            throw new Exception(
                ...
                Mobilpay_Payment_Request_Abstract::ERROR_FACTORY_BY_XML_ORDER_TYPE_ATTR_NOT_FOUND
            );
        }
        switch ($attr->nodeValue) {
            ...
            default:
                throw new Exception(
                    'factoryFromXml invalid payment request type='.$attr->nodeValue,
                    Mobilpay_Payment_Request_Abstract::ERROR_FACTORY_BY_XML_INVALID_TYPE
                );
                break;
        }
    ...
    protected function _parseFromXml(DOMNode $elem)
    {
        ...
        if ($xmlAttr == null || strlen((string)$xmlAttr->nodeValue) == 0) {
            throw new Exception(
                'Mobilpay_Payment_Request_Sms::_parseFromXml failed: empty order id', self::ERROR_LOAD_FROM_XML_ORDER_ID_ATTR_MISSING
            );
        }
