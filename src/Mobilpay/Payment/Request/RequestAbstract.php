...
abstract class RequestAbstract
{
     ...
     * Mandatory for Mobilpay\Payment\Request\Sms
     * Optional for Mobilpay\Payment\Request\Card
     */
    public $service = null;

    /**
     * orderId (Mandatory)    - payment transaction identifier generated by merchant; helps merchant to interpret a
     * request to confirm or return url; it should be unique for the specified signature
     *
     * @var string(64)
     */
    public $orderId = null;
    ...
    static public function factoryFromEncrypted($envKey, $encData, $privateKeyFilePath, $privateKeyPassword = null, $cipher_algo = 'RC4', $iv = null)
    {
        ...
        if ($privateKeyPassword == null) {
            $privateKey = @openssl_get_privatekey(
                is_file($privateKeyFilePath) && is_readable(
                    $privateKeyFilePath
                ) ? "file://{$privateKeyFilePath}" : $privateKeyFilePath
            );
        } else {
            $privateKey = @openssl_get_privatekey(
                is_file($privateKeyFilePath) && is_readable(
    ...
    public function encrypt($x509FilePath)
    {
        ...
        $publicKey = openssl_pkey_get_public(
            is_file($x509FilePath) && is_readable($x509FilePath) ? file_get_contents($x509FilePath) : $x509FilePath
        );
        if ($publicKey === false) {
